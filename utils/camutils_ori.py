...
def cam_to_label2(cam, cls_label, img_box=None, bkg_thre=None, high_thre=None, low_thre=None, ignore_mid=False,
                  ignore_index=None):
    b, c, h, w = cam.shape
    # pseudo_label = torch.zeros((b,h,w))
    cls_label_rep = cls_label.unsqueeze(-1).unsqueeze(-1).repeat([1, 1, h, w])
...
def multi_scale_cam2_siamese(model, inputs1, inputs2, scales, branch=1):
    '''process cam and aux-cam'''
    # cam_list, tscam_list = [], []
    b, c, h, w = inputs1.shape
    with torch.no_grad():
        inputs_cat = torch.cat([inputs1, inputs1.flip(-1)], dim=0)
        inputs_cat2 = torch.cat([inputs2, inputs2.flip(-1)], dim=0)

        _cam_aux, _cam = model(inputs_cat, inputs_cat2, cam_only=True, branch=branch)
        ...
        for s in scales:
            if s != 1.0:
                _inputs1 = F.interpolate(inputs1, size=(int(s * h), int(s * w)), mode='bilinear', align_corners=False)
                inputs_cat1 = torch.cat([_inputs1, _inputs1.flip(-1)], dim=0)

                _inputs2 = F.interpolate(inputs2, size=(int(s * h), int(s * w)), mode='bilinear', align_corners=False)
                inputs_cat2 = torch.cat([_inputs2, _inputs2.flip(-1)], dim=0)

                _cam_aux, _cam = model(inputs_cat1, inputs_cat2, cam_only=True, branch=branch)
...
def cams_to_affinity_label(cam_label, mask=None, cut=32, ignore_index=255):
    b, h, w = cam_label.shape

    cam_label_resized = F.interpolate(cam_label.unsqueeze(1).type(torch.float32), size=[h // cut, w // cut],
                                      mode="nearest")
...
def label_to_aff_mask(cam_label, ignore_index=255):
    ...
    # 沿第二个维度重复，形状变为(b, h * w, h * w)。这样，每个像素的标签信息都与图像中所有其他像素的标签信息并排
    _cam_label_rep = _cam_label.repeat([1, _cam_label.shape[-1], 1])

    _cam_label_rep_t = _cam_label_rep.permute(0, 2, 1)

    # 使用标签矩阵的对比结果来生成亲和力标签 aff_label。当两个像素标签相同时，aff_label 中对应位置的值为 1，否则为 0。
    aff_label = (_cam_label_rep == _cam_label_rep_t).type(torch.long)
...
def refine_cams_with_bkg_v2(ref_mod=None, images=None, cams=None, cls_labels=None, high_thre=None, low_thre=None,
                            ignore_index=False, img_box=None, down_scale=2, background=True):
...
def Choose_refine_cams_with_bkg(ref_mod=None, images=None, cams=None, cls_labels=None, high_thre=None, low_thre=None,
                                ignore_index=False, img_box=None, down_scale=2, background=True):
    b, _, h, w = images.shape
    _images = F.interpolate(images, size=[h // down_scale, w // down_scale], mode="bilinear", align_corners=False)
