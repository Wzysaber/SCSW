...
from utils.losses import get_masked_ptc_loss, get_seg_loss, CTCLoss_neg, DenseEnergyLoss, get_energy_loss, JointLoss, \
    ContrastiveCorrelationLoss, NT_Xent0, get_aff_loss
...
# from model_test.DSENet.TSCD_model import TSCD
...
parser.add_argument("--GPU_num", default=4, type=int, help="number of GPU")
parser.add_argument("--information", default='ALL  (0.7 0.6)(0.5, 1.0, 1.5, 2), 2e-5 (0.5 0.5) ', type=str,
                    help="information")
...
parser.add_argument("--data_folder",
                    default="/home0/students/master/2022/wangzy/datasets/Deepglobe_WSSS/deepglobe_512_512/",
                    type=str, help="dataset folder")
parser.add_argument("--list_folder",
                    default="/home0/students/master/2022/wangzy/Pycharm-Remote(161)/WSSS_Model2/datasets/deepglobe/",
                    type=str,
...
parser.add_argument("--work_dir",
                    default="/home0/students/master/2022/wangzy/Pycharm-Remote(161)/WSSS_Model2/weight/Deepglobe_dataset/",
...
def validate(device, model=None, data_loader=None, args=None):
    ...
    with torch.no_grad():
        for _, data in tqdm(enumerate(data_loader), total=len(data_loader), ncols=100, ascii=" >="):
            ...
            cam_label = cam_to_label2(resized_cam, cls_label, bkg_thre=args.bkg_thre, high_thre=args.high_thre,
    ...
    seg_score = evaluate.scores2(gts, preds, 7)
    cam_score = evaluate.scores2(gts, cams, 7)

    model.train()

    tab_results, mIoU = format_tabs([cam_score, seg_score], name_list=["CAM", "Seg_Pred"],
                                    cat_list=deepglobe.class_list)
...
def train(args=None):
    ...
    #
    model = TSCD('mit_b1', stride=[4, 2, 2, 1], pooling="gmp", num_classes=7, embedding_dim=256, pretrained=True)
    ...
    for n_iter in range(args.max_iters):
        ...
        # seg_loss & reg_loss
        valid_cam, pseudo_label = cam_to_label2(0.5 * cams1.detach() + 0.5 * cams2.detach(), cls_label=cls_label,
        ...
        if n_iter <= 4000:
            ...
            loss = 1.0 * cls_loss + 1.0 * aux_cls_loss + args.w_seg * seg_loss + args.w_reg * reg_loss + 0.1 * loss_er + 0.1 * loss_corr + 0.3 * aux_loss
