...
def conv5x5(in_planes, out_planes, stride=1, dilation=1, padding=1):
    " 5 x 5 conv"
    return nn.Conv2d(in_planes, out_planes, kernel_size=5, stride=stride, padding=padding, dilation=dilation,
                     bias=False)
...
class LargeFOVModified(nn.Module):
    def __init__(self, in_planes, out_planes, dilation=5):
        ...
        self.conv3x3_1 = conv3x3(in_planes=in_planes, out_planes=self.embed_dim, padding=self.dilation,
                                 dilation=self.dilation)
        self.relu3x3_1 = nn.ReLU(inplace=True)

        self.conv3x3_2 = conv3x3(in_planes=self.embed_dim, out_planes=self.embed_dim, padding=self.dilation,
                                 dilation=self.dilation)
        self.relu3x3_2 = nn.ReLU(inplace=True)

        self.conv5x5 = nn.Conv2d(in_channels=self.embed_dim, out_channels=self.embed_dim, kernel_size=5, padding=2,
                                 dilation=self.dilation)
...
class LargeFOVWithDilation(nn.Module):
    def __init__(self, in_planes, out_planes, dilations=[3, 5]):
        ...
        self.dilated_conv1 = conv3x3(in_planes=self.embed_dim, out_planes=self.embed_dim, padding=dilations[0],
                                     dilation=dilations[0])
        self.dilated_conv2 = conv3x3(in_planes=self.embed_dim, out_planes=self.embed_dim, padding=dilations[1],
