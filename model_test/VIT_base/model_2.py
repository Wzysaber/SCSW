...
class network(nn.Module):
    def __init__(self, backbone, num_classes=None, pretrained=True, init_momentum=None, aux_layer=-3):
        super().__init__()
    ...
    def forward(self, x, cam_only=False):
        ...
        _x4 = self.to_2D(x_patch, h, w)
        _x_aux = self.to_2D(x_aux_patch, h, w)

        attn_weights_list = [attn_weights[0], attn_weights[3], attn_weights[7], attn_weights[11]]

        if cam_only:
            n, b, h, w = _x4.shape

            init_cams = F.conv2d(_x4, self.classifier.weight).detach()
            ...
            attn_weights_no_cls = attn_weights_list.sum(0)[:, 1:, 1:]

            conv_flat = init_cams.flatten(start_dim=2)

            cam = torch.einsum('nhw,ncw->nch', attn_weights_no_cls, conv_flat).reshape(
                [n, self.num_classes - 1, h, w])
            cams = init_cams + cam
...
if __name__ == '__main__':
    ...
    cls_x4, seg = model(image, cam_only=True)
