...
from utils.losses import get_masked_ptc_loss, get_seg_loss, CTCLoss_neg, DenseEnergyLoss, get_energy_loss, JointLoss, \
    ContrastiveCorrelationLoss, get_aff_loss, attn_aff
...
parser.add_argument("--GPU_num", default=6, type=int, help="number of GPU")
parser.add_argument("--information",
                    default='mit_b1 + aux +pre+ 强弱监督0.1（双loss）+ Losser + 4阶段aux_loss 0.1 (对应100-400，并将aff dim=C concat  4阶段)(Avg)(cam1)(0.6 0.4) seed=1',
                    type=str,
                    help="information")
...
parser.add_argument("--list_folder",
                    default="/home0/students/master/2022/wangzy/Pycharm-Remote(161)/CTFA/datasets/iSAID/",
                    type=str,
...
parser.add_argument("--work_dir",
                    default="/home0/students/master/2022/wangzy/Pycharm-Remote(161)/WSSS_Model2/weight/ISAID_dataset/",
...
def train(args=None):
    ...
    for n_iter in range(args.max_iters):
        ...
        crops = crops[1].to(device, non_blocking=True)
        ...
        cams2 = F.interpolate(cams2,  size=cams1.shape[2:], mode='bilinear',
                             align_corners=True)
        segs2 = F.interpolate(segs2,  size=segs1.shape[2:], mode='bilinear',
                             align_corners=True)
        ...
        refined_pseudo_label = refine_cams_with_bkg_v2(par, inputs_denorm, cams=valid_cam, cls_labels=cls_label,
                                                       ...
                                                       ignore_index=args.ignore_index, img_box=img_box, background=True)
