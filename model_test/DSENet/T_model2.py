...
class TSCD(nn.Module):
    def __init__(self, backbone, num_classes=None, embedding_dim=256, stride=None, pretrained=None, pooling=None):
        ...
        self.CAM_ALL_Fusion = CAM_ALL_Fusion(
            in_features=[self.in_channels[0], self.in_channels[1], self.in_channels[2], self.in_channels[3]])
        self.Pseudo_classifier = nn.Conv2d(in_channels=self.in_channels[0], out_channels=self.num_classes - 1,
                                           kernel_size=1,
                                           bias=False)
    ...
    def get_param_groups(self):
        ...
        param_groups[2].append(self.Pseudo_classifier.weight)
        ...
        for name, param in self.CAM_ALL_Fusion.named_parameters():
            param_groups[2].append(param)
    ...
    def forward(self, x, cam_only=False, aux=False):
        ...
        Pseudo_Map = self.CAM_ALL_Fusion(_x)

        if cam_only:
            cam = F.conv2d(Pseudo_Map, self.Pseudo_classifier.weight).detach()
            return cam
        ...
        return final_x, aux_cls, segs, _x


if __name__ == "__main__":
    model = TSCD('mit_b1', stride=[4, 2, 2, 1], pooling="gmp", num_classes=16, embedding_dim=256, pretrained=False)

    dummy_input = torch.rand(1, 3, 512, 512)

    cls_x4, aux_cls, segs, _x4 = model(dummy_input, cam_only=True)
