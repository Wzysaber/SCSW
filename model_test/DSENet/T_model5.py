...
class siamese_network(nn.Module):
    def __init__(self, backbone="deit_base_patch16_224",
                 pretrained=False,
                 num_classes=16,
                 init_momentum=0.9):
        super().__init__()
    ...
    def get_param_groups(self):
        ...
        for proj in self.branch1.StageAttn_projs:
            param_groups[2].append(proj.weight)
            param_groups[2].append(proj.bias)

        for name, param in self.branch1.Aux_x.named_parameters():
            param_groups[2].append(param)
        for name, param in self.branch2.Aux_x.named_parameters():
            param_groups[2].append(param)
