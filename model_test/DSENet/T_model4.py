...
class TSCD(nn.Module):
    def __init__(self, backbone, num_classes=None, embedding_dim=256, stride=None, pretrained=None, pooling=None):
        ...
        # self.decoder = Single_SegFormerHead(in_planes=self.in_channels[3], out_planes=self.num_classes)
        ...
        self.StageAttn_projs = nn.ModuleList([
            ...
            nn.Conv2d(in_channels=10, out_channels=1, kernel_size=1, bias=True),
        ...
        self.classifier = nn.Conv2d(in_channels=self.in_channels[3], out_channels=self.num_classes, kernel_size=1,
                                    bias=False)
    ...
    def forward(self, x, cam_only=False, aux=False):
        ...
        attn_preds = []
        for i in range(4):
            # attn_mean = torch.mean(torch.stack(attns_groups[i], dim=0), dim=0)
        ...
        final_x = final_x.view(-1, self.num_classes)

        aux_cls = self.pooling(mid_x, (1, 1))
        aux_cls = self.aux_classifier(aux_cls)
        aux_cls = aux_cls.view(-1, self.num_classes)
...
if __name__ == "__main__":
    model = TSCD('mit_b1', stride=[4, 2, 2, 1], pooling="gmp", num_classes=7, embedding_dim=256, pretrained=False)
