...
from model_test.MCTformer.vision_transformer import VisionTransformer, _cfg
...
class MCTformerPlus(VisionTransformer):
    def __init__(self, decay_parameter=0.996, input_size=320, *args, **kwargs):
        ...
        self.decoder = LargeFOV(384, 16)

    def Train_interpolate_pos_encoding(self, x, w, h):

        dim = x.shape[-1]
        w0 = w // self.patch_embed.patch_size[0]
        h0 = h // self.patch_embed.patch_size[0]
        patch_pos_embed = nn.Parameter(torch.zeros(1, w0 * h0, dim))
    ...
    def forward_features(self, x, n=12):
        ...
        if not self.training:
            ...
            pos_embed_pat = self.Train_interpolate_pos_encoding(x, w, h)
            x = x + pos_embed_pat
    ...
    def forward(self, x, n_layers=12, attention_type='fused', cam_only=False, return_att=True):
        w, h = x.shape[2:]
        ...
        xmap = x_patch.contiguous()
        x_patch = self.head(xmap)
        ...
        weights = torch.logspace(start=0,
        ...
        #     return cams
        # else:
        #     return x_cls_logits, x_patch_logits, segs
...
if __name__ == '__main__':
    ...
    image = torch.randn(8, 3, 320, 320)

    x_logits, cams, patch_attn = model(image, return_att=True)
    print("input:", image.shape)
    print("output:", x_logits.shape)
    print("output:", cams.shape)
    print("output:", patch_attn.shape)
