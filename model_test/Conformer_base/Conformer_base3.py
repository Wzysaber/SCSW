...
class Net(nn.Module):
    ...
    def __init__(self, patch_size=16, in_chans=3, num_classes=1000, base_channel=64, channel_ratio=4, num_med_block=0,
        ...
        self.Tranpatch_cls_head = nn.Conv2d(384, 16, 1, bias=False)
        self.Tran_head = nn.Conv2d(384, 16, 1, bias=False)
    ...
    def forward(self, x, cam_only=False):
        ...
        if cam_only:
            ...
            x_conv = F.conv2d(x, self.conv_cls_head.weight).detach()
        ...
        tranpatch_logits = self.Tranpatch_cls_head(x_patch)
        tranpatch_logits = self.pooling(tranpatch_logits).flatten(1)

        trans_logits = trans_logits + tranpatch_logits
        ...
        cams = torch.einsum('nhw,ncw->nch', attn_weights_no_cls, conv_flat).reshape([n, 16, h, w])
        Tran_cam = self.Tran_head(x_patch)

        # seg
        segs = self.decoder(x)

        return conv_logits, trans_logits, x_conv, Tran_cam, segs
