...
class Net(nn.Module):
    ...
    def __init__(self, patch_size=16, in_chans=3, num_classes=1000, base_channel=64, channel_ratio=4, num_med_block=0,
        ...
        self.trans_cls_head = nn.Linear(embed_dim, 6, 1)
        self.pooling = nn.AdaptiveAvgPool2d(1)
        # self.conv_cls_head = nn.Conv2d(int(256 * channel_ratio), self.num_classes, kernel_size=3, stride=1, padding=1)
        self.conv_cls_head = nn.Conv2d(int(256 * channel_ratio), 6, 1, bias=False)
        ...
        self.decoder = LargeFOV(1024, self.num_classes)
    ...
    def forward(self, x, cam_only=False):
        ...
        if cam_only:
            ...
            cams = torch.einsum('nhw,ncw->nch', attn_weights_no_cls, conv_flat).reshape(
                [n, 6, h, w])
        ...
        cams = torch.einsum('nhw,ncw->nch', attn_weights_no_cls, conv_flat).reshape([n, 6, h, w])
...
if __name__ == '__main__':
    model = Conformer_base6()
    print(model)
    model.eval()
    image = torch.randn(5, 3, 512, 512)

    output1, output2, output3, output4, output5 = model(image)
