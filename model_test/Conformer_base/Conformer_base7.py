...
class conformer(nn.Module):
    ...
    def forward(self, x, cam_only=False):
        ...
        return x, x_t, attn_weight, Data_conv_x


class Conformer_base(nn.Module):
    def __init__(self, num_classes=16, embed_dim=384, channel_ratio=4):
        super().__init__()
        self.forward_features = conformer(patch_size=16, channel_ratio=channel_ratio, embed_dim=embed_dim, depth=12,
        ...
        self.conv_cls_head = nn.Conv2d(int(256 * channel_ratio), num_classes, 1, bias=False)
        self.fusion_conv = conv1x1(int(256 * channel_ratio) + embed_dim, int(256 * channel_ratio))
        self.fusion_head = nn.Conv2d(int(256 * channel_ratio), num_classes, 1, bias=False)

        self.patch_cls_head = nn.Conv2d(embed_dim, num_classes, 1, bias=False)

        # segs
        self.decoder = LargeFOV(1024, num_classes)

    def forward(self, x, cam_only=False):
        x, x_t, attn_weight, Data_conv_x = self.forward_features(x)
        ...
        # fusion_map
        # fusion_map_data = torch.cat([x, x_patch], dim=1)
        # fusion_map_data = self.fusion_conv(fusion_map_data)
        # fusion_map = self.fusion_head(fusion_map_data)
        # fusion_logit = self.pooling(fusion_map).flatten(1)

        # # tran_patch_logit
        # patch_map = self.patch_cls_head(x_patch)
        # patch_logits = self.pooling(patch_map).flatten(1)
        ...
        return conv_logits, trans_logits, fmap, segs


if __name__ == '__main__':
    model = Conformer_base(num_classes=6)
    print(model)
    model.eval()
    image = torch.randn(5, 3, 512, 512)

    output1, output2, output3, output4 = model(image, cam_only=False)
